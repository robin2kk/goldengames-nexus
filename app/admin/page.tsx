...
import { getAdminAuthResult, isAdminConfigured } from "../cloudflare-access-auth";
...
export default async function AdminPage() {
  ...
  if (!user) {
    return <AdminShell><div className="admin-card access-card" data-access-validator="jose-v2"><LockKeyhole size={30}/><h1>Private Admin Panel</h1><p>{isAdminConfigured() ? "Your authenticated account is not authorized to manage GoldenGames Nexus." : "Cloudflare Access must be configured before this area can be used in production."}</p><small>Reference: ACCESS-{auth.reason.toUpperCase()}</small><a className="primary-btn" href="/">Return to the website</a></div></AdminShell>;
  }
